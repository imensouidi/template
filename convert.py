...
CORS(
    app,
    resources={
        r"/*": {"origins": ["https://talent.heptasys.com", "http://localhost:4200"]}
    },
...
azure_openai_client = AzureOpenAI(
    api_key=api_key, api_version="2024-02-15-preview", azure_endpoint=azure_endpoint
)
...
IMPORTANT: Do not include any extra symbols; provide only a valid JSON output.
    ...
    try:
        response = azure_openai_client.completions.create(
            model="IndexSelector", prompt=prompt, max_tokens=3000
        )
...
# Génération du nom du PDF à partir du fichier d'entrée avec "_output" ajouté à la fin
...
def generate_pdf_from_json(json_data, output_file):
    ...
    styles.add(
        ParagraphStyle(
            name="Section", fontSize=16, textColor=colors.turquoise, spaceAfter=12
        )
    )
    styles.add(
    ...
    def draw_banner(canvas_obj, doc_obj):
        try:
            ...
            canvas_obj.drawImage(
                banner_img, 0, A4[1] - banner_height, width=A4[0], height=banner_height
            )
        except Exception as e:
            logging.error(f"Error loading banner image: {e}")
        canvas_obj.setFillColor(colors.white)
        job_title = (
            json_data.get("job_title", "").replace("\n", " ").strip() or "CV Title"
        )
        years_experience = (
            str(json_data.get("years_of_experience", "")).replace("\n", " ").strip()
        )
    ...
    if isinstance(skills_section, dict) and skills_section:
        for category_key, skills in skills_section.items():
            ...
            # Les noms de catégories sont affichés en gras
            story.append(
                Paragraph(f"<b>{category_title}</b> {skills_text}", styles["Normal"])
            )
        ...
        if extracted_text:
            ...
            if json_info:
                ...
                json_file_path = os.path.join(
                    tempfile.gettempdir(), "extracted_info.json"
                )
